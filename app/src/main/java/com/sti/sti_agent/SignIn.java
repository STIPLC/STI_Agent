...
import com.sti.sti_agent.Model.Auth.LoginModel.UserPostData;
...
import static com.sti.sti_agent.operation_fragment.MotorInsurance.MotorInsureFragment1.isValidEmailAddress;
...
public class SignIn extends AppCompatActivity implements View.OnClickListener{
    ...
    @BindView(R.id.inputLayoutBrokersEmail)
    TextInputLayout inputLayoutBrokersEmail;
    @BindView(R.id.brokers_email_editxt)
    EditText brokers_email_editxt;

    @BindView(R.id.b_card)
    MaterialCardView b_card;
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.a_card:
                mInputLayoutAgentEmail.setClickable(true);
                mInputLayoutAgentEmail.setVisibility(View.VISIBLE);
                inputLayoutBrokersEmail.setClickable(false);
                inputLayoutBrokersEmail.setVisibility(View.GONE);
                a_card.setClickable(false);
                a_card.setAlpha(0.7f);
                b_card.setClickable(true);
                b_card.setAlpha(1.0f);
                break;

            case R.id.b_card:
                inputLayoutBrokersEmail.setClickable(true);
                inputLayoutBrokersEmail.setVisibility(View.VISIBLE);
                mInputLayoutAgentEmail.setClickable(false);
                mInputLayoutAgentEmail.setVisibility(View.GONE);
                b_card.setClickable(false);
                b_card.setAlpha(0.7f);
                a_card.setClickable(true);
                a_card.setAlpha(1.0f);
                break;
        }
    ...
    private void ValidateForm() {
        ...
        if (networkConnection.isNetworkConnected(this)) {
            ...
            if (mAgentEmailEditxt.getText().toString().isEmpty()&&mInputLayoutAgentEmail.isClickable()) {
                mInputLayoutAgentEmail.setError("Agent Email is required!");
                isValid = false;
            ...
            if (brokers_email_editxt.getText().toString().isEmpty()&&inputLayoutBrokersEmail.isClickable()) {
                inputLayoutBrokersEmail.setError("Broker Email is required!");
                isValid = false;
            } else if (!isValidEmailAddress(brokers_email_editxt.getText().toString())&&inputLayoutBrokersEmail.isClickable()) {
                inputLayoutBrokersEmail.setError("Valid Email is required!");
                isValid = false;
            } else {
                inputLayoutBrokersEmail.setErrorEnabled(false);
            }
